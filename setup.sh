...
echo "🕐 Проверяем наличие установленного Carthage версии ${CARTHAGE_VERSION}..."

if which carthage >/dev/null; then
	CURRENT_CARTHAGE_VERSION=$(swiftlint version)
	if [ $CURRENT_CARTHAGE_VERSION != $CARTHAGE_VERSION ]; then
		brew upgrade carthage
		echo "✅ Обновление до версии $(CARTHAGE_VERSION)"
	else
		brew switch carthage $CARTHAGE_VERSION
		echo "✅ Установлена актуальная версия Carthage"	
	fi
else
	echo "Установка Carthage"
...
echo "🕞 Проверяем наличие установленного XCProfiler..."
if which xcprofiler >/dev/null; then
	echo "✅ Установлена актуальная версия XCProfiler"
else
...
echo "🕔 Обновляем зависимости из Bundler'a..."
bundle update
